...
**致谢**：使用 vue-bits / reactbits 源码时，在页面 footer 或 README 注明 "Motion effects derived from vue-bits by DavidHDev (MIT)"。
...
> 注意：以下 `references/` 和 `scripts/` 目录未随本次导入一并添加，仅有本 SKILL.md 主文件。涉及这些路径的步骤在本项目中暂不可用，可作为指导性说明参考。
