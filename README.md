...
The client and API deploy as a single Vercel project:

- `vercel.json` builds the client (`npm run build` → `dist/`) and rewrites
  `/api/*` to `api/index.js`, a serverless function that wraps the same
  Express app used by `npm start` — no route/service code is duplicated.
- Because a Vercel Function's filesystem is read-only outside of `/tmp`,
  `DB_PATH` automatically points at `/tmp/schemesaathi.db` when the
  `VERCEL` environment variable is set (see `server/env.js`). The function
  seeds the scheme catalogue on the first request of a cold start if it
  finds the database empty.
- `CORS_ORIGINS` automatically includes the deployment's own
  `VERCEL_URL`/`VERCEL_PROJECT_PRODUCTION_URL`, so same-origin cookies work
  without extra config — set `CORS_ORIGINS` yourself only if you add a
  custom domain and it doesn't show up in `VERCEL_URL`.

Set these Project → Environment Variables before deploying:

| Variable            | Required | Notes                                                        |
| -------------------- | -------- | ------------------------------------------------------------ |
| `JWT_SECRET`          | Yes      | The server refuses to boot in production without it.         |
| `SEED_DEMO_USERS`     | No       | Set to `1` to get the `asha`/`ravi`/`officer`/`admin` demo logins on the deployment (off by default in production). |
| `GROQ_API_KEY`        | No       | Enables the chat widget; it degrades gracefully without one. |

Then `vercel deploy` (or connect the repo in the dashboard) — no other
configuration is needed.

**Important caveat:** `/tmp` is wiped on every cold start and isn't shared
across concurrent function instances, so this setup is a working demo, not
durable production storage — registrations, applications, and admin edits
can disappear the next time the instance restarts or a request lands on a
different instance. For real persistence, point `server/db` at a hosted
database reachable over the network (e.g. Turso/libSQL, or Postgres via
`DATABASE_URL`) instead of a local SQLite file; `server/db/index.js` is the
only place that would need to change, since routes and services only ever
call its `query`/`one`/`run`/`tx` helpers.
